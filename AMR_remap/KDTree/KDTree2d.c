...
static void median_sort2d(TKDTree2d* t,
                        int cut_direction, int k, int num, int* idx)
{
   ...
   for (left = 0, right = num - 1; (right - left) > 1;) {
      ...
      if(Bounds_CenterAxis2d(&(t->elements[idx[left + 1]]), cut_direction) >
         Bounds_CenterAxis2d(&(t->elements[idx[right]]), cut_direction))
         SWAP(idx[left + 1], idx[right], int);
      if(Bounds_CenterAxis2d(&(t->elements[idx[left]]), cut_direction) >
         Bounds_CenterAxis2d(&(t->elements[idx[right]]), cut_direction))
         SWAP(idx[left], idx[right], int);
      if(Bounds_CenterAxis2d(&(t->elements[idx[left + 1]]), cut_direction) >
         Bounds_CenterAxis2d(&(t->elements[idx[left]]), cut_direction))
      ...
      while (1) {
         for (i++;
              Bounds_CenterAxis2d(&(t->elements[idx[i]]), cut_direction) <
                Bounds_CenterAxis2d(&(t->elements[a]), cut_direction);
              i++);
         for (j--;
              Bounds_CenterAxis2d(&(t->elements[idx[j]]), cut_direction) >
              Bounds_CenterAxis2d(&(t->elements[a]), cut_direction);
              j--);
         if(j < i)
            break;
...
void KDTree_Destroy2d(TKDTree2d* t)
{
   assert(t);
   /* Flush the overall tree extent */
   Bounds_Infinite2d(&(t->extent));
...
void KDTree_AddElement2d(TKDTree2d* t, TBounds2d* add)
{
   assert(t && add);
   ...
   if(t->elements_num == t->elements_allocated) {
      t->elements_allocated += KDTREE_ELEMENT_BLOCKING_SIZE;
      t->elements = REALLOC(t->elements, t->elements_allocated, TBounds2d);
      assert(t->elements);
   }
   /* Add the new element to the overall extent and the element list */
   Bounds_AddBounds2d(&(t->extent), add);
   Bounds_Copy2d(add, &(t->elements[t->elements_num]));
   t->elements_num++;
}

void KDTree_CreateTree2d(TKDTree2d* t)
{
   ...
   if(t->elements_num > 0) {
      ...
      t->tree_safety_boxes = MALLOC(t->tree_size, TBounds2d);
      ...
      Bounds_Copy2d(&(t->extent), &(t->tree_safety_boxes[0]));
      next_node++;
      ...
      while (stack_ptr) {
         ...
         if ((max - min) == 0) {
            Bounds_Copy2d(&(t->elements[idx[min]]), &(t->tree_safety_boxes[parent]));
         ...
         for (i = 0; i < 2; i++) {
            width = Bounds_WidthAxis2d(&(t->tree_safety_boxes[parent]), i);
            if(width > max_width) {
               max_width = width;
               cut_direction = i;
            }
         ...
         /* Do a median sort of the elements under the parent node. The sort key
            ...
            cutting direction. */
         mid = (min + max) /2;
         median_sort2d(t, cut_direction, mid - min, max - min + 1, &(idx[min]));
         ...
         Bounds_Infinite2d(&(t->tree_safety_boxes[next_node]));
         for (i = min; i <= mid; i++) {
            Bounds_AddBounds2d(&(t->tree_safety_boxes[next_node]),
                             &(t->elements[idx[i]]));
         }
         next_node++;
...
void KDTree_QueryBoxIntersect2d(TKDTree2d* t,
   ...
   *result_num = 0;
   ...
   while (stack_ptr) {
      ...
      /* Check if the query box intersects an epsilon-expanded safety box for
         the current node. */
      Bounds_Copy2d(&(t->tree_safety_boxes[node]), &sb);
      ...
      if(!Bounds_IsOverlappingBounds2d(&sb, box))
         continue;
